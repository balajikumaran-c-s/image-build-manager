...
Top-level build configuration. Synced to `<clone_path>/config.yml` on the target.

| Field | Description | Example |
|-------|-------------|---------|
| `project_name` | Project name for input/output paths | `project_default` |
| `host.hostname` | Short hostname (NOT FQDN) | `abhoim` |
| `host.shared_path` | Persistent storage for MinIO, registry, logs | `/opt/omnia/image_build_manager` |
| `host.domain_name` | Domain suffix. Registry = hostname.domain:5000 | `vm.cluster` |
| `host.admin_nic_ip` | Admin NIC IP — Pulp and S3 endpoint | `10.20.0.1` |
