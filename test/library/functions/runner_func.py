...
def run_playbook(
    ...
    Args:
        ...
        verbosity: Ansible verbosity 0-4 (default from runner_vars).
        timeout: Max seconds to wait (default from runner_vars).
    ...
    v = verbosity if verbosity is not None else DEFAULT_VERBOSITY
    ...
    log = TestLogger(RUNNER_LOGGER_NAME)
    ...
    return _stream_cmd(cmd, playbook, t, tag, config)
...
def _build_ansible_cmd(
    ...
    parts = [
        f"cd {workdir} &&",
        f"COLUMNS={LINE_WIDTH} ansible-playbook {playbook}{v_flag}",
    ]
...
def _print_wrapped(line: str, prefix: str) -> None:
    """Print a line, wrapping at LINE_WIDTH."""
    while line:
        chunk = line[:LINE_WIDTH]
