...
|------|-------------|--------|
| **Mode A (bare-metal)** | Runs directly on the RHEL host via `ansible-playbook` | **Recommended** |
| Mode B (per-domain container) | Each domain runs inside its own container | **Not recommended** — host path sharing and privilege escalation issues make this fragile |
| Mode C (Omnia core container) | All domains run inside a shared `omnia_core` container | **Not recommended** — single container creates coupling, version conflicts, and complicates host/container path translation |
...
- **Air-gap friendly** — no container runtime needed for the playbook itself (Podman is used only for image builds)

Mode B/C code is **commented out** in the codebase, not deleted, to preserve reference for future consideration.
...
├── CODING_RULES.md                # Domain-specific coding rules (extends this doc)
...
host:
  hostname: "myhost"               # Short hostname (NOT FQDN), alphanumeric + hyphens
  shared_path: "/opt/omnia/<domain_name>"  # Absolute path — persistent storage
  domain_name: "local"             # Domain suffix
  admin_nic_ip: "10.20.0.1"       # Admin NIC IPv4 address
```
...
| Path | Purpose |
|------|---------|
| `<shared_path>/` | Root persistent storage |
