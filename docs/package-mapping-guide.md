...
From `FUNCTIONAL_GROUP_LAYER_MAP` in `library/module_utils/build_image/config.py`:
...
## Relationship to Mode C (Historical)

In Mode C (Omnia mono-repo, **NOT SUPPORTED**), package resolution used:
...
The Mode C code is commented out in `fetch_build_packages/tasks/main.yml` for reference.
