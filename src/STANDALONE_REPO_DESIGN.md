...
that works in three execution modes: bare-metal (Ansible+Python), per-domain container, or
inside the Omnia mono-repo ecosystem. The core container is **not required**.
...
build_host:
  hostname: "localhost"                     # "localhost" for local builds, IP for remote
  ssh_user: "root"
  ssh_port: 22
  shared_path: "/opt/image_build"           # local or NFS path for build artifacts
  domain_name: "local"
  admin_nic_ip: "10.20.0.1"                 # Admin NIC IP (used for MinIO endpoint)
...
log_path = /opt/omnia/log/core/playbooks/image_build_manager.log
